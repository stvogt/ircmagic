...
def bondOrder(filelines):
  ...
  for lineNum in range(0,len(filelines)):
    ...
    if "Wiberg bond index matrix in the NAO basis:" in outline:
      for lineNum2 in range(lineNum+1,len(filelines)):
        ...
        if atm:
          ...
          bndorder.append(bnd)
        if  "Wiberg bond index, Totals by atom:" in filelines[lineNum2]:
          break
  #bnd = filter(None,temp)
  for i in range(0,len(atoms)):
    for j in range(i+1,len(bndorder[i])):
    #if float(bndorder[i][j]) > 0.4:
      #print "The bond order of the "+atoms[i]+"-"+atoms[j]+" bond is:    "+bndorder[i][j]
